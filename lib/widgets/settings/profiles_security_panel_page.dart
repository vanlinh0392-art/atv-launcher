...
class _ProfilesSecurityPanelPageState extends State<ProfilesSecurityPanelPage> {
  ...
  Widget build(BuildContext context) {
    return Consumer<ProfileSecurityService>(
      ...
      child: Column(
        ...
        children: [
          ...
          SettingsActionCard(
            ...
            onPressed: () async => _openManager(context, hiddenMode: true),
          ),
          const SizedBox(height: TvDrawerTokens.rowSpacing),
          SettingsActionCard(
            icon: Icons.lock_outline,
            title: localizations.manageLockedAppsAction,
            onPressed: () async => _openManager(context, hiddenMode: false),
          ),
        ],
      ),
    );
...
class _LauncherSecurityCard extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
          ...
          RoundedSwitchListTile(
            ...
            onChanged: (value) => _toggleSettingsLock(context, value),
            title: Text(
              localizations.settingsLockTitle,
          ...
          SettingsActionCard(
            ...
            onPressed: () async => _upsertOwnerPin(context),
          ),
          const SizedBox(height: TvDrawerTokens.rowSpacing),
          SettingsActionCard(
            icon: Icons.no_encryption_gmailerrorred_outlined,
            title: localizations.clearOwnerPinAction,
            onPressed: security.hasPin ? () async => _clearOwnerPin(context) : null,
          ),
        ],
      ),
    );
  ...
  Future<void> _toggleSettingsLock(BuildContext context, bool value) async {
    final localizations = AppLocalizations.of(context)!;
    if (!security.hasPin) {
      if (!value) {
        await security.setSettingsLockEnabled(false);
        return;
      }
      ...
      final success = await security.setOwnerPin(newPin);
      if (!context.mounted) {
        return;
      }
      if (!success) {
      ...
      await security.setSettingsLockEnabled(true);
      return;
    }
    ...
    await security.setSettingsLockEnabled(value);
  }
