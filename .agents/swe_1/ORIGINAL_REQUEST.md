...
- Test suite chính thức của dự án: flutter test (chứa 282+ bài kiểm thử tích hợp và kiểm thử giao diện).
- Kịch bản kiểm thử vòng đời đánh thức TV: scripts/smoke_wallpaper_wake.py.
- Lệnh kiểm tra phân tích tĩnh: flutter analyze.
...
- [ ] 100% test cases trong flutter test vượt qua màu xanh (All tests passed!).
- [ ] flutter analyze không có bất kỳ lỗi biên dịch nghiêm trọng nào.
- [ ] Build thành công file APK release armeabi-v7a (flutter build apk --release --target-platform=android-arm --split-per-abi) với dung lượng tối ưu ~22.8 MB.
