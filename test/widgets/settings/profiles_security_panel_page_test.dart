...
void main() {
  ...
  testWidgets('renders app security page in the detail pane without overflow',
    ...
    final hiddenAppsFrame = tester.widget<SettingsFocusFrame>(
      find.ancestor(
        of: find.text('Manage hidden apps'),
        matching: find.byType(SettingsFocusFrame),
      ),
    );
    expect(hiddenAppsFrame.variant, SettingsFocusFrameVariant.rowOnly);
  });

  testWidgets('renders app security page in Vietnamese without overflow',
      (tester) async {
    tester.view.physicalSize = const Size(1280, 720);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    expect(find.text('Đặt PIN chủ máy'), findsOneWidget);
    expect(find.text('Xóa PIN chủ máy'), findsOneWidget);
    expect(tester.takeException(), isNull);
  });

  testWidgets('owner PIN dialogs stay within the viewport on TV layouts',
      (tester) async {
    tester.view.physicalSize = const Size(1280, 720);
    tester.view.devicePixelRatio = 1.0;
    addTearDown(tester.view.resetPhysicalSize);
    ...
    final sharedPreferences = await SharedPreferences.getInstance();
    final security = ProfileSecurityService(sharedPreferences);
    await security.setOwnerPin('1234');
    final appsService = MockAppsService();
    when(appsService.applications).thenReturn(<App>[
      App(
        packageName: 'com.example.video',
    ...
    await tester.tap(find.text('Clear owner PIN'));
    await tester.pumpAndSettle();
    ...
    await tester.tap(find.text('Change owner PIN'));
    await tester.pumpAndSettle();
